...
func LoadCompactCandidates(
	db *sql.DB,
) ([]CompactCandidate, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf(
				"scan compact candidate: %w",
				err,
			)
		}

		row.FirstExpiry, err = time.Parse(
			"2006-01-02",
			expiry[:10],
		)
